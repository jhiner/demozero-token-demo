...
const getGoogleConnectionId = async () => {
  ...
  try {
    const response = await makeApi2Request(requestOptions);
    const googleConnection = response.filter((connection) => connection.name === GOOGLE_CONNECTION_NAME);

    if (googleConnection.length === 0) {
      throw new Error('Unable to create org. Google connection not found, so cannot enable the connection.');
    }
...
const getSAMLAppClient = async () => {
  ...
  try {
    ...
    const matchingClient = response.filter((client) => client.name === CLIENT_NAME_FOR_SAML_DEMO_APP);
