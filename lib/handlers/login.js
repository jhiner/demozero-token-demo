...
const authenticateWithOrganization = (req, res) => {
  ...
  if (req.query.opt_param_connection) {
    authorizeUrl = `${authorizeUrl}&connection=${req.query.opt_param_connection}`;
  }

  if (req.query.opt_param_login_hint) {
    authorizeUrl = `${authorizeUrl}&login_hint=${req.query.opt_param_login_hint}`;
  }
